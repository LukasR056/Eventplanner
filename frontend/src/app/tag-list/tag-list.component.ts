...
export class TagListComponent implements OnInit {
  ...
  changeEvent(tag: any) {
    ...
    this.showEvents = this.events.filter(event => event.tags.includes(tag.id) && event.public && event.eventplanner != this.userId && !event.participants.includes(Number(this.userId)) && !event.invited.includes(Number(this.userId)));
